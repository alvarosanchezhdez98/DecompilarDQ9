...
`tools/complete_file.py` does both: it finds the file's code and variables in the ROM, writes its sections in `delinks.txt`, marked complete, and gives the symbols in `symbols.txt` the compiler's names, marking the static ones `local`. It also moves the relocations to symbols inside a variable, and writes the sizes that dsd needs. When none of the file's functions is named like `symbols.txt` yet, give it where the code starts:
```shell
